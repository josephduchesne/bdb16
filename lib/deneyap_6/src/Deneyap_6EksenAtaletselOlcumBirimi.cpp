...
status_t LSM6DSMCore::beginCore(uint8_t address) {
    ...
    if (readCheck != WHO_AM_I_REG_VALUE) {
        Serial2.printf("WHO_AM_I error: 0x%02x != 0x6A\n", readCheck);
...
LSM6DSM::LSM6DSM() {
    ...
    settings.gyroSampleRate = 104;    // Hz: 13, 26, 52, 104, 208, 416, 833, 1666, 3330, 6660
    ...
    settings.accelSampleRate = 104;    // Hz: 1, 13, 26, 52, 104, 208, 416, 833, 1666, 3330, 6660
    ...
    // FIFO control data
    settings.fifoThreshold = 12; // 0 to 4096 (16 bit bytes)
    settings.fifoSampleRate = 104;  // Default 10Hz
...
status_t LSM6DSM::begin(uint8_t address, SensorSettings *pSettingsYouWanted) {
    ...
    if (returnError != IMU_SUCCESS) {
        Serial2.println("Error: beginCore");
        return returnError;
    }
    ...
    if (returnError != IMU_SUCCESS) {
        Serial2.println("Error: CTRL1_XL");
        return returnError;
    }
...
void LSM6DSM::fifoBegin(void) {
    ...
    tempFIFO_CTRL5 |= settings.fifoModeWord = 0b110; // set mode: Continuous FIFO mode

    // Write data
    uint8_t status = 0;
    status |= writeRegister(FIFO_CTRL1, thresholdLByte);
    status |= writeRegister(FIFO_CTRL2, thresholdHByte);
    status |= writeRegister(FIFO_CTRL3, tempFIFO_CTRL3);
    status |= writeRegister(FIFO_CTRL4, tempFIFO_CTRL4);
    status |= writeRegister(FIFO_CTRL5, tempFIFO_CTRL5);
    Serial2.printf("fifoBegin status: 0x%02x\n", status);
}
